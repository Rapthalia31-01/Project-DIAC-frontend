...
import { AsignaturaList, Course, Destreza, Objective, Criteria } from '../../interfaces/asignatura.interface';
...
export class VerAsignaturasComponent implements OnInit {
  ...
  activateAnimationArrowCri: boolean = false
  ...
  valueDelete!: number
  valueUpdate!: Destreza | Objective | Criteria
  ...
  rotateArrowCri() {
    this.activateAnimationArrowCri = !this.activateAnimationArrowCri
  }
  ...
  async getCriteria(obj: Criteria) {
    this.isDestreza = false
    this.valueUpdate = obj
    const { value: Criterio, isConfirmed } = await Swal.fire({
      title: 'Actualizar Criteria',
      input: 'text',
      inputLabel: 'Criterio',
      inputValue: obj.nameCriteria,
      confirmButtonText: 'Guardar Cambios',
      showCancelButton: true
    })
    if (isConfirmed) {
      if (Criterio !== '') {
        this.valueInputUpdate = Criterio
        this.updateDestrezaObjetivo()
      } else {
        Swal.fire('El objetivo no puede quedar vacio', '', 'error')
      }
  ...
  deleteCriterio(id: number) {
    ...
    this.valueDelete = id
    Swal.fire({
      title: '¿Esta seguro que desea eliminar este criterio?',
      icon: 'info',
      showDenyButton: true,
      confirmButtonText: 'Eliminar',
      denyButtonText: `Cancelar`,
  ...
  updateDestrezaObjetivo() {
    if (this.isDestreza) {
      ...
      this.destrezaService.updateCriterio(this.valueUpdate.id, { nameCriteria: this.valueInputUpdate })
        .subscribe({
          next: (CriteriaResponse) => {
            const newArrCriterios = this.asignaturaSeleccionada!.Criterias.map(criterio => {
              if (criterio.id === CriteriaResponse.id) {
                criterio.nameCriteria = CriteriaResponse.nameCriteria
              }
              return criterio
            })
            this.asignaturaSeleccionada!.Criterias = newArrCriterios
            Swal.fire('Criterio actualizado', '', 'success')
          },
          error: (error) => {
            Swal.fire('Hubo un error al actualizar el criterio', '', 'error')
          }
  ...
  deleteDestrezaObjetivo() {
    if (this.isDestreza) {
      ...
      this.destrezaService.deleteCriterio(this.valueDelete)
        .subscribe({
          next: (CriteriaResponse) => {
            const newArrCriterios= this.asignaturaSeleccionada!.Criterias.filter(criterio => {
              return criterio.id !== CriteriaResponse.id
            })
            this.asignaturaSeleccionada!.Criterias = newArrCriterios
            Swal.fire('Criterio eliminado', '', 'success')
          },
          error: () => {
            Swal.fire('Hubo un error al eliminar el criterio', '', 'error')
          }
        })
    }
  }
